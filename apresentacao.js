...
module.exports = function gerarFaturaStr(fatura, calc) {
  ...
  for (let apre of fatura.apresentacoes) {
    faturaStr += `  ${calc.repo.getPeca(apre).nome}: ${formatarMoeda(calc.calcularTotalApresentacao(apre))} (${
      apre.audiencia
    } assentos)\n`
  }
...
//   for (let apre of fatura.apresentacoes) {
//     faturaHtml += `<li>  ${calc.repo.getPeca(apre).nome}: ${formatarMoeda(calc.calcularTotalApresentacao(apre))} (${
//       apre.audiencia
//     } assentos)\n`
//   }
